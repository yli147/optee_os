...
static uint32_t plic_get_interrupt_enable(struct plic_data *pd, uint32_t source)
{
	uint32_t hartid = get_core_pos();
	uint32_t current = 0;
...
static uint32_t plic_get_threshold(struct plic_data *pd)
{
	uint32_t hartid = get_core_pos();
...
static uint32_t plic_get_priority(struct plic_data *pd, uint32_t source)
{
	uint32_t priority = 0;
